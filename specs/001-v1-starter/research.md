...
Jackson Databind stays on the compile-scope dependency list because (a) PRD §12 lists it for completeness and most adopters already pull it through `spring-boot-starter-web`, and (b) Phase 6's JSON body masking helper may use Jackson's tokenizer for keys-only scanning. The Phase 2 deliverable does NOT couple to Jackson; that decision is revisited at Phase 6 only if a regex approach proves insufficient.
