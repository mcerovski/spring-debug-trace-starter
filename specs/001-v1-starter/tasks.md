...
- [ ] T005 [P] Add a `TraceLogs` helper or a documented constant for the dedicated SLF4J logger name `io.github.cerovskimatija.debugtrace` in `src/main/java/io/github/cerovskimatija/debugtrace/TraceLogs.java`. Library classes obtain their logger via `LoggerFactory.getLogger("io.github.cerovskimatija.debugtrace")` (the fixed string, NOT `getClass()`). Per research.md R11.
...
- [ ] T006 [P] Create `src/main/java/io/github/cerovskimatija/debugtrace/properties/DebugTraceProperties.java` annotated `@ConfigurationProperties("spring-debug-trace")` with a top-level `enabled: boolean` (default `false`) and `format: String` (default `"pretty"`). Nested static classes: `Method`, `Http`, `TraceId`, `Serialization`, `Masking` — all initially empty placeholders to be grown per phase. Constructor injection, `private final`, full Javadoc. Per contracts/configuration-properties.md, plan.md Principle VII.
...
- [ ] T020 [US1] Extend `DebugTraceProperties` with the Phase 1 keys: `baseAcksases` aka `basePackages: List<String>` (default `[]`) and `excludedPackages: List<String>` (default `[]`); `Method` nested with `enabled: boolean` (default `true`). Add validation: trim entries, drop nulls. Per contracts/configuration-properties.md top-level + `method.enabled`.
...
- [ ] T032 [US3] Add an IT case where the masked DTO's getter throws: assert request completes 200/expected status and the log contains `<serialization-failed: ...>` (SC-010).
...
- [ ] T058 [P] [US6] Add the header masking path inside `SensitiveDataMasker`: an immutable `Set<String> headerNames` constructed as defaults (`authorization, cookie, set-cookie`) ∪ the `fieldNames` set, all lowercased. Method `boolean shouldMaskHeader(String name)`.
